...
| store.write_csv            | No       | false           | When true, two CSV files are created (or overwritten) storing comprehensive metadata: `albums_metadata.csv` (album details: AlbumKey, Title, Description, Keywords, dates, privacy, image count, etc.) and `images_metadata.csv` (image/video details: filename, title, caption, keywords, dimensions, format, GPS coordinates, dates, etc.). This provides a complete export of all SmugMug data for archival purposes.                                                                                                                                                                                       |
